...
####################################
# Dataset for ADE20K instance segmentation
####################################
...
class ADE20KDataset(CustomDataset):
    ...
    CLASSES = ('bed', 'windowpane', 'cabinet', 'person', 'door', 'table', 'curtain', 'chair', 'car',
               'painting', 'sofa', 'shelf', 'mirror', 'armchair', 'seat', 'fence', 'desk', 'wardrobe',
               'lamp', 'bathtub', 'railing', 'cushion', 'box', 'column', 'signboard', 'chest of drawers',
               'counter', 'sink', 'fireplace', 'refrigerator', 'stairs', 'case', 'pool table', 'pillow',
               'screen door', 'bookcase', 'coffee table', 'toilet', 'flower', 'book', 'bench', 'countertop',
               'stove', 'palm', 'kitchen island', 'computer', 'swivel chair', 'boat', 'arcade machine', 'bus',
               'towel', 'light', 'truck', 'chandelier', 'awning', 'streetlight', 'booth', 'television receiver',
               'airplane', 'apparel', 'pole', 'bannister', 'ottoman', 'bottle', 'van', 'ship', 'fountain', 'washer',
               'plaything', 'stool', 'barrel', 'basket', 'bag', 'minibike', 'oven', 'ball', 'food', 'step',
               'trade name', 'microwave', 'pot', 'animal', 'bicycle', 'dishwasher', 'screen', 'sculpture',
    ...
    def summarize(self,cocoEval):
        '''
        Compute and display summary metrics for evaluation results.
        Note this functin can *only* be applied on the default parameter
        setting
        '''
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100, F1=False):
            p = cocoEval.params
        ...
        def _summarizeDets():
            ...
            stats[11] = _summarize(0,
                                   areaRng='large',
                                   maxDets=cocoEval.params.maxDets[2])
            stats[12] = _summarize(1,F1=True)
            stats[13] = _summarize(1, iouThr=.5, maxDets=cocoEval.params.maxDets[2],F1=True)
            stats[14] = _summarize(1,
                                  iouThr=.75,
                                  maxDets=cocoEval.params.maxDets[2],F1=True)
            return stats
